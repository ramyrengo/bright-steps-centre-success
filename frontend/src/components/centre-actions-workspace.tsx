...
export function CentreActionWorkspace({ actionId }: Readonly<{ actionId: string }>) {
  ...
  async function acknowledge() {
    if (!action || action.finding.origin.source !== "QUARTERLY_AUDIT") return;
    setWorking(true);
    try { await client.foundation.acknowledgeQuarterlyAudit(action.finding.origin.auditId, { comment: "Reviewed in Centre Success." }); setWarning("Quarterly review acknowledged."); await load(); }
    catch { setError(true); }
    finally { setWorking(false); }
  }
