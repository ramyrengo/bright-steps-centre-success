...
describe("operational template API surface", () => {
  test("exposes only authenticated resource-oriented Centre Standards routes", () => {
    ...
    expect(source.match(/= api\(/gu)).toHaveLength(11);
    expect(source.match(/expose:\s*true,\s*auth:\s*true/gu)).toHaveLength(11);
    ...
    expect(source).toContain('path: "/standards/templates/:templateId/draft-from-version"');
