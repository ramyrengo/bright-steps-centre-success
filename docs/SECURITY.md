...
Privileged packages require a distinct current System Administrator to approve the exact immutable package. Every access mutation is attributable and preserves at least one reachable active System Administrator under concurrency; operations target two. Administrative APIs require normal provisioned AuthData plus PostgreSQL authority. Only sensitive `POST /invitations/accept` admits an unmapped invitation candidate: it reuses the strict Entra verifier but never creates internal AuthData or confers business access. Invitation email delivery uses a PostgreSQL outbox and idempotent Encore Pub/Sub worker. ADR-0016 approves Microsoft Graph only for the fixed-mailbox staging send adapter, never for guest discovery, provisioning, directory access, or business authority. See `PEOPLE_AND_ACCESS.md`, ADR-0014 and ADR-0016.

### Staging Microsoft Graph invitation-email boundary

- Runtime selection uses trusted Encore metadata. Only `cloud = encore`, `name = staging`, `type = development` loads the Graph adapter or its credential. Local/test uses a no-network adapter; previews, other development environments and Production fail closed.
- `MicrosoftGraphClientSecret` is an Encore secret scoped only to exact staging. `EntraTenantId` and `EntraApiClientId` supply the public identifiers. No secret, app-only token, raw provider body, invitation code, email address, `tid`, `oid`, role or scope is logged or added to audit context.
- Client-credentials token acquisition is tenant-specific, uses only `https://graph.microsoft.com/.default`, has bounded timeouts, expiry-skewed in-memory reuse and singleflight refresh. A send `401` invalidates the cache and permits exactly one fresh-token resend; a second `401` is terminal. A `403` invalidates the cache and is retryable only within the generation's hard attempt cap. The implementation does not require a token `roles` claim and does not add broad Entra `Mail.Send` API permission.
- The only Graph resource is `POST /v1.0/users/centresuccess@brightstepsacademy.com.au/sendMail`. Sender address/display name are constants. Exchange Application RBAC remains scoped to that mailbox; the proven forbidden-mailbox `403` is a required least-privilege invariant.
- Only `202` records `microsoft-graph:accepted`. The payload omits `message.from`, fixes `replyTo`, and sets `saveToSentItems = false` so the live code is not copied into Sent Items. Retry delay accepts only valid non-negative `Retry-After` values and clamps at ten minutes. A separate provider-reservation row is durably committed before Graph is reachable. The database serializes numbering on the outbox and accepts only contiguous reservations 1–3 per generation; a crash, rollback, timeout or ambiguous provider response never deletes or decrements one. Stale reservations become `AMBIGUOUS`, and reservation-three exhaustion records terminal `delivery_attempts_exhausted`. Graph bodies are cancelled/discarded rather than persisted or exposed. A lost accepted response may yield duplicate mail within the three-reservation bound because Graph provides no provider idempotency key; exactly-once delivery is not claimed.
- Dispatch uses an expiring database lease before Pub/Sub publication. Publish failure returns work to `PENDING` only by matching that lease, so a later worker result cannot be overwritten. The periodic dispatcher reclaims stale `PUBLISHED` leases after process crash or ambiguous publication; duplicate messages remain bounded by the provider-reservation invariant.
- After committing the reservation, the worker re-locks and revalidates invitation status, current generation, expiry and reservation. It then holds the invitation row lock/database transaction through the bounded token/send call and reconciliation. That intentional staging/pilot trade-off provides deterministic cancel/resend/current-generation ordering while making the attempt cap crash durable. Any future redesign must preserve pre-call durable reservation, the hard cap, expiry, cancellation, resend and current-generation invariants.
- Resend enforces `expires_at <= trusted now` under the invitation lock. An elapsed `SENT` invitation and current token generation become `EXPIRED`; no replacement generation or outbox row is created.
...
- no break-glass, impersonation, automated employee provisioning, HR synchronisation, Graph directory/authorisation/provisioning integration, or Entra-owned business authorisation is introduced; ADR-0016's staging invitation send is the sole Graph exception.

Milestone 2C People & Access is **ACCEPTED / COMPLETE** within the approved architecture: migrations, the invitation acceptance boundary, APIs, the outbox/Pub/Sub worker, provider-neutral development delivery, and frontend routes are present. ADR-0016 later adds the staging Graph adapter without making this a production-readiness declaration. Production email enablement/provider, the production first-administrator ceremony, template/support operations, Entra email-claim operations, retention, rate/abuse controls, JML source/SLA, access reviews, and break-glass/recovery remain deferred decisions.
