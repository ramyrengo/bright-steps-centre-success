...
The local/test development adapter is deliberately deterministic and performs no network or email delivery. Send/resend still exercises the real transaction, HMAC verifier record, AES-GCM outbox ciphertext, Pub/Sub dispatch boundary, idempotency and delivery-attempt state. After a successful terminal delivery, the implementation clears the ciphertext, IV and authentication tag while retaining non-sensitive delivery metadata; retryable/unpublished/terminal-failed rows retain the complete encrypted tuple for controlled support/recovery under the future retention policy. Never put role/scope details in an email.

### Staging Microsoft Graph invitation delivery

ADR-0016 approves Graph only when Encore runtime metadata is exactly `cloud = encore`, `name = staging`, `type = development`. Local and test remain no-network. Ephemeral previews, any differently named development environment and Production are disabled even if a secret exists.

The adapter reuses `EntraTenantId` and `EntraApiClientId` and declares one new secret:

```sh
encore secret set --env staging MicrosoftGraphClientSecret
```

Do not run that command until the implementation has passed review and the Product Owner supplies the value out of band. Never use `--type dev`, `--type pr`, `--type prod`, or a local value for this credential. Diagnose configuration without exposing values using `encore secret list MicrosoftGraphClientSecret`; the reviewed outcome must name only exact `staging`. CI's secret guard deliberately fails until that exact environment configuration exists.

The fixed sender is `Bright Steps Centre Success <centresuccess@brightstepsacademy.com.au>`. Exchange enterprise application object `93017243-f438-43ee-a3d9-01f85fa27be8` has Application RBAC `Application Mail.Send` scoped only to that mailbox. Do not add Graph `Mail.Send` under Entra API Permissions and do not configure a second sender. The already observed control evidence is approved sender `202` and an unrelated mailbox `403`; do not repeat the unrelated-mailbox test unless separately authorised.

After targeted re-review, secret configuration and reviewed deployment, create a newly approved synthetic invitation and observe its existing outbox through dispatch/delivery. Do not resend the previously delivered planner invitation. Expected delivery metadata is `microsoft-graph:accepted`; no Graph access token, credential, recipient address, invitation code, provider response body, `tid`, `oid`, role or scope may appear in application logs or audit context. The Graph payload uses `saveToSentItems = false`, has no `message.from`, and keeps fixed mailbox `replyTo`. A send `401` invalidates cache and permits one fresh-token retry within the same provider reservation; a second `401` is terminal. `403` records safe `graph.mailbox_authorization` and, with `429`, `5xx`, network interruption and timeout, is retryable only within three durably committed provider reservations per invitation generation. Reservations 1–3 are database-serialized and committed before Graph is reachable. A crash, result-transaction rollback or ambiguous accepted response consumes its slot; stale work becomes `AMBIGUOUS`. Attempts one and two may reschedule using a valid non-negative `Retry-After` clamped at ten minutes; reservation-three exhaustion becomes terminal `delivery_attempts_exhausted`. Explicit administrator resend may rotate a new generation only while the current invitation is unexpired. Because Graph offers no sendMail idempotency key, a lost response can cause duplicate email within that three-reservation bound; the implementation does not claim exactly-once delivery.

The staging worker commits its provider reservation first, then opens a new transaction and revalidates invitation status, current generation, `expires_at` and reservation under the invitation lock. It intentionally holds that lock/transaction across the bounded token/send operation so cancel/resend cannot overtake current-generation delivery. This is accepted for staging/pilot volume. Monitor lock duration and contention; any future claim/call/reverify redesign must preserve the durable pre-call reservation, three-slot cap, expiry, current-generation, cancellation, resend and duplicate-delivery safety.

The dispatcher writes a `PUBLISHED` lease before Pub/Sub publication. Its compare-and-set failure path cannot overwrite a worker's later result, and the five-minute dispatcher cron automatically reclaims expired leases after a crash before/after publish. At-least-once duplicate publication is expected and safe only because the worker enforces active reservation/current state and the database attempt cap. A resend at or after `expires_at` materialises both invitation and current generation as `EXPIRED` and creates no new outbox work.

Production delivery remains separately deferred, as do template ownership, monitoring/support procedures, retention, and any broader notification provider decision.

Non-blocking follow-ups are permanently-failed ciphertext retention, HTML/plain-text multipart content, GUID-validator generalisation, and lock-duration monitoring before broader volume.
