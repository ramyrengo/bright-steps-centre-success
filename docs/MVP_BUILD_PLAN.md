...
**Milestone 2 — started under controlled sub-milestone gates.** Milestones 2A and 2B are accepted and complete. **Milestone 2C — People & Access + User Invitations** is **IMPLEMENTED — ACCEPTANCE REMEDIATION IN PROGRESS** and is not yet accepted. Later milestones remain locked.
...
**Status:** **IMPLEMENTED — ACCEPTANCE REMEDIATION IN PROGRESS.** The Product Owner accepted Milestone 2B and separately authorised Milestone 2C implementation on 11 August 2026, limited to the approved People & Access scope in `docs/PEOPLE_AND_ACCESS.md` and ADR-0014. Forward migrations 016–018, workflow APIs, the candidate-only acceptance boundary, PostgreSQL outbox/Pub/Sub delivery seam, last-reachable-administrator invariant, task-focused Next.js routes and deterministic tests are implemented. An independent review returned PASS WITH CHANGES; this contained remediation closes the identified performance, operator-safety, portfolio-editing, approval-legibility, canonical-package and focused-test findings without changing the accepted security core. This evidence does not pre-empt targeted independent re-review or Product Owner acceptance. The concrete production email provider and production first-administrator mechanism remain separately gated.
